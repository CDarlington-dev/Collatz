...
/-!
# A universal affine remainder bound

For a length-`j` accelerated segment with `q` odd inputs, its endpoint has
linear coefficient `3^q / 2^j`.  The additive numerator is largest when all
even steps precede all odd steps.  This file proves that assertion directly by
induction, without assuming that the extremal parity word is realizable.

The resulting exact integer inequality rules out every paradoxical segment
above start `4614` at lengths below `27`.  This is useful preprocessing for a
proof-producing bounded-model certificate: no circuit needs to assert the
paradoxical predicate at the first 26 prefixes.
-/
...
/-- Largest possible additive numerator for `j` steps and `q` odd inputs. -/
...
/--
Exact scaled affine bound for every realizable accelerated parity prefix.
-/
...
private def shortBoundCheck : Bool :=
  (List.range 27).all fun j =>
    (List.range (j + 1)).all fun q =>
      decide (3 ^ q < 2 ^ j →
        maxRemainder j q < (2 ^ j - 3 ^ q) * 4615)

private theorem shortBoundCheck_true : shortBoundCheck = true := by
  decide
...
theorem paradoxical_length_at_least_27 {n j : ℕ}
    (hn : 4614 < n) (hp : Paradoxical n j) : 27 ≤ j := by
  by_contra hj
  exact no_paradoxical_short_above_4614 hn (by omega) hp

private def mediumBoundCheck : Bool :=
  (List.range 35).all fun j =>
    (List.range (j + 1)).all fun q =>
      decide (3 ^ q < 2 ^ j →
        maxRemainder j q < (2 ^ j - 3 ^ q) * 26018)

private theorem mediumBoundCheck_true : mediumBoundCheck = true := by
  decide

theorem medium_numeric_bound {j q : ℕ} (hj : j ≤ 34) (hq : q ≤ j)
    (hfactor : 3 ^ q < 2 ^ j) :
    maxRemainder j q < (2 ^ j - 3 ^ q) * 26018 := by
  have hallj : ∀ x, x ∈ List.range 35 →
      (List.range (x + 1)).all fun y =>
        decide (3 ^ y < 2 ^ x →
          maxRemainder x y < (2 ^ x - 3 ^ y) * 26018) = true := by
    simpa [mediumBoundCheck] using mediumBoundCheck_true
  have hjmem : j ∈ List.range 35 := by
    simp only [List.mem_range]
    omega
  have hallq := hallj j hjmem
  have hqmem : q ∈ List.range (j + 1) := by simpa using hq
  have hcheck : decide (3 ^ q < 2 ^ j →
      maxRemainder j q < (2 ^ j - 3 ^ q) * 26018) = true := by
    simpa using (List.all_eq_true.mp hallq q hqmem)
  exact (of_decide_eq_true hcheck) hfactor

theorem no_paradoxical_through_34_above_26017 {n j : ℕ}
    (hn : 26017 < n) (hj : j ≤ 34) : ¬ Paradoxical n j := by
  intro hp
  have hq := oddCount_le n j
  have hupper := start_bound_of_paradoxical hp
  have hlower := medium_numeric_bound hj hq hp.factor_lt_one
  have hn26018 : 26018 ≤ n := by omega
  have hmul := Nat.mul_le_mul_left (2 ^ j - 3 ^ oddCount n j) hn26018
  omega

theorem paradoxical_length_at_least_35 {n j : ℕ}
    (hn : 26017 < n) (hp : Paradoxical n j) : 35 ≤ j := by
  by_contra hj
  exact no_paradoxical_through_34_above_26017 hn (by omega) hp
